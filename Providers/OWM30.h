...
#ifndef _OPENWEATHERMAP_30_PROVIDER_H_
#define _OPENWEATHERMAP_30_PROVIDER_H_
...
static const QString OWM_30_PROVIDER = "OpenWeatherMap 3.0 API";

/** \class OWM30Provider
 * \brief Weather provider that uses OpenWeatherMap 3.0 API.
 *
 */
class OWM30Provider
: public WeatherProvider
{
    Q_OBJECT
  public:
    /** \brief OWM30Provider class constructor.
     * \param[in] config Application configuration information.
     *
     */
    OWM30Provider(Configuration &config)
    : WeatherProvider(OWM_30_PROVIDER, config)
    {
      loadSettings();
    };

    /** \brief OWM30Provider class virtual destructor.
     *
     */
    virtual ~OWM30Provider()
    {
      saveSettings();
    };

    virtual ProviderCapabilities capabilities() const override
    { return ProviderCapabilities(true, true, true, true, true, true, true, true); }
  ...
  private:
    const QString OPENWEATHERMAP_APIKEY = QString("OpenWeatherMap 3.0 API Key");
